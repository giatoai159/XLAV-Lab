...
int Converter::HSV2RGB(const Mat& sourceImage, Mat& destinationImage)
{
	...
	for (int y = 0; y < height; y++)
	{
		...
		for (int x = 0; x < width; x++, pSourceRow += sourceChannels, pDestinationRow += destinationChannels)
		{
			...
			float R, G, B;
			float C = V * S;
			float X = (float)(C * (1 - abs(fmod(H / 60.0f, 2) - 1))); // X = C * (1 - |H' mod 2 - 1|), H' = H/60;
			float m = V - C;
			if (H >= 0 && H < 60) {
				R = C, G = X, B = 0;
			}
...
int Converter::Convert(Mat& sourceImage, Mat& destinationImage, int type)
{
	switch (type)
	{
	case 0: if (RGB2GrayScale(sourceImage, destinationImage)) return 1; break;
	case 1: if (GrayScale2RGB(sourceImage, destinationImage)) return 1; break;
	case 2: if (RGB2HSV(sourceImage, destinationImage)) return 1; break;
	case 3: if (HSV2RGB(sourceImage, destinationImage)) return 1; break;
	default: return 1;
	}
	return 0;
}
